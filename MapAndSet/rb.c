...
// ----------------------------------------------------------------------------//
int DealDupMultiSet(void *x,void *key)
{
    Pair *keykey=key;
    Pair *xx=x;
    xx->value ++;
    printf(" already in the RB tree\n");
    return 0;
    //xx->value=keykey->value;
}
RbTreePtr MultiSetInsert(RbTreePtr T, int keyword) // int (*cmp)(const void *, const void *))  // insert <key,value> pair)
{
    Pair *x=MakePair(keyword,1); // occurance initial is  // occurance initial is 11
    DataInsert(T,x,(int (*)(const void *,const void *))&cmp_map,(int (*)(const void *,const void *))&DealDupMultiSet);
}

RbTreePtr MultiSetErase(RbTreePtr T, int keyword)
{
    Pair *key=MakePair(keyword,0); //value is arbitray
    RbTreePtr x;
    Pair *y;
    //if ((x=RbSearch(T,key))!=&nilNode)  // can find key
    if((x=RbSearch(T,key,&cmp_map))!=&nilNode  ) // can find key
    {
          y=(Pair *)x->key;
         if(y->value >1)
             y->value --;
...
int main()
{
    ...
    printf("find %d\n",value);
    T2=MultiSetInsert(T2,1);
    T2=MultiSetInsert(T2,2);
    T2=MultiSetInsert(T2,1);
    T2=MultiSetInsert(T2,3);
    T2=MultiSetInsert(T2,5);
    T2=MultiSetInsert(T2,3);
    printf("xxxxx----------------------------------------------------------\n");
    TreeBFS(T2);
    printf("xxxx----------------------------------------------------------\n");
